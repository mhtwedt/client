...
void LibCloudProvidersPrivate::updateFolderExport()
{
    for (auto folderExport : _folderExports) {
        ...
        case SyncResult::Paused:
            status = CLOUD_PROVIDERS_ACCOUNT_STATUS_IDLE; // questionable
            break;
        }

        // We don't do this in other status reporting, but it would make some sense?
        //if (!account->_folder->accountState()->isConnected())
        //    status = CLOUD_PROVIDERS_ACCOUNT_STATUS_ERROR;
